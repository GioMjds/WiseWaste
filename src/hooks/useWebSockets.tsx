...
import { WebSocketConnectEvent, WebSocketEvent, EventHandler, EventHandlers, WebSocketDisconnectEvent, WebSocketChannelJoinEvent, WebSocketChannelLeaveEvent } from "@/types/useWebSocketsTypes";
...
export interface WebSocketService {
    ...
    on: <T extends WebSocketEvent['type']>(eventType: T, handler: EventHandler<T>) => void;
...
const useWebSockets = (
    ...
    const handleConnect = useCallback((event: WebSocketConnectEvent) => {
        setIsConnected(true);
    ...
    const handleDisconnect = useCallback((event: WebSocketDisconnectEvent) => {
    ...
    // Channel management
    const joinChannel = useCallback(async (channelName: string): Promise<boolean> => {
    ...
    const leaveChannel = useCallback(async (channelName: string): Promise<boolean> => {
    ...
    const handleChannelJoin = useCallback((event: WebSocketChannelJoinEvent) => {
    ...
    const handleChannelLeave = useCallback((event: WebSocketChannelLeaveEvent) => {
        if (event.success) {
            setConnectedChannels(prev => prev.filter(ch => ch !== event.channel));
        }
    ...
    useEffect(() => {
        ...
        (Object.entries(allHandlers) as Array<[WebSocketEvent['type'], EventHandler<WebSocketEvent['type']>]>)
        ...
        return () => {
            ...
            (Object.keys(allHandlers) as WebSocketEvent['type'][])
                .forEach(eventType => {
    ...
    const send = useCallback((data: object, channel?: string): boolean => {
        ...
        if (channel && !connectedChannels.includes(channel)) {
            console.warn(`Attempting to send message to channel ${channel} that is not connected`);
            return false;
        }
